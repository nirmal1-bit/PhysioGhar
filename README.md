...
- `Provider<T>` — exposes stable dependencies such as Dio, repositories, and
  network services.
- `FutureProvider<T>` — exposes one-time asynchronous dependencies such as
  `SharedPreferences` and `SessionService`.
- `AsyncNotifierProvider` — manages feature state that can load asynchronously
  and later be changed by user actions.
- `FutureProvider.family` — loads data for a specific identifier, such as a
  patient details page.

### AsyncNotifier state flow

Feature controllers use `AsyncValue` states:
...
repository request
    ├── AsyncData(value)
    └── AsyncError(error, stack trace)
```

This gives screens a consistent way to render loading, data, error, retry, and
empty states. Mutations update the controller state after a successful API
response so the current screen reflects the server result immediately.

### Authentication and session state

`AuthController` is the central authentication controller. It:

- logs in and saves the returned token and backend-provided user type;
- clears old credentials before a new login;
- exposes the current authentication request state;
- logs out by clearing in-memory state and persisted token/role;
- prevents a patient session from being reused by a therapist session.

`SessionService` owns local session storage. Screens do not read or write
`SharedPreferences` directly. The `AuthInterceptor` reads the persisted token
for protected API requests, while backend authorization remains the source of
...
Screen callback
    ↓
Riverpod controller method
    ↓
Domain repository contract
    ↓
Data repository implementation
    ↓
BaseRemoteSource + Dio
    ↓
FastAPI backend
    ↓
Typed response or AppError
    ↓
Controller state update
    ↓
UI feedback and refreshed content
```

This separation keeps screens focused on presentation, keeps API conversion in
repositories, and makes loading/error behavior consistent across features.
